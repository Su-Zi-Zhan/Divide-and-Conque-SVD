% .report/report/tex

\documentclass[12pt]{article}
 
\usepackage[margin=1in]{geometry} 
\usepackage{amsmath,amsthm,amssymb}
\usepackage{tcolorbox}
\usepackage{graphicx}
\usepackage[utf8]{inputenc}
\usepackage{listings}
\usepackage{hyperref}
\usepackage{float}
\usepackage[linesnumbered,ruled,vlined]{algorithm2e}
\SetKw{Continue}{continue}
\SetKw{Break}{break}

 
\newcommand{\N}{\mathbb{N}}
\newcommand{\Z}{\mathbb{Z}}
\newcommand{\R}{\mathbb{R}}
\newcommand{\C}{\mathbb{C}}
\newcommand{\im}{\mathrm{i}}
\newcommand{\ex}{\mathrm{e}}
\newcommand{\op}{\operatorname}
\newcommand{\ep}{\varepsilon}
\newcommand{\prg}{\paragraph}
 
\newtheorem{theorem}{Theorem}[section]
\newtheorem{corollary}[theorem]{Corollary}
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{observation}[theorem]{Observation}
\newtheorem{proposition}[theorem]{Proposition}
\newtheorem{definition}[theorem]{Definition}
\newtheorem{claim}[theorem]{Claim}
\newtheorem{fact}[theorem]{Fact}
\newtheorem{assumption}[theorem]{Assumption}
\newtheorem{example}[theorem]{Example}
\newtheorem*{solution}{Solution}
\lstset{
    language=Python,
    basicstyle=\ttfamily\small,
    numbers=left,
    numberstyle=\tiny,
    stepnumber=1,
    numbersep=5pt,
    showstringspaces=false,
    breaklines=true,
    frame=single,
    frameround=tttt,
    captionpos=b,
    keywordstyle=\color{blue},
    identifierstyle=\color{black},
    commentstyle=\color{gray!80!black}\textit,
    stringstyle=\color{red}
}

\tcbuselibrary{breakable}
 
\begin{document}
 
% --------------------------------------------------------------
%                         Start here
% --------------------------------------------------------------
 
\title{Project Report}
\author{24300720157 Ruijie Li}

\maketitle

\tableofcontents
\newpage

\section{Introduction}

The Singular Value Decomposition (SVD) is a cornerstone of numerical linear algebra, with applications spanning data compression, signal processing, and machine learning. While the standard Golub--Kahan algorithm is numerically stable, its sequential nature limits performance on modern parrallel architectures. This led to the development of Divide-and-Conquer SVD algorithms (Cuppen, 1981), which recursively break the problem into subproblems, allowing for parallel execution and improved efficiency.

However, the naive implementation of the Divide-and-Conquer strategy suffers from a critical numerical instability. When singular values are closely spaced, the standard formulas for updating singular vectors are extremely sensitive to round-off errors, leading to significant inaccuracies in the computed SVD.

Fortunately, Gu and Eisenstat (1995) proposed a bidiagonal Divide-and-Conquer algorithm (BDC) that addresses these stability issues. Instead of directly computing the singular vectors from the original rank-1 perturbed system, their algorithm modifies the diagonal elements slightly to ensure that the secular equation's roots are well-separated. This adjustment significantly enhances numerical stability, allowing for accurate computation of singular values and vectors even in challenging scenarios. The BDC algorithm computes all the eigenvalues in \(O(n^2)\) time and the singular vectors in \(O(n^3)\) time, making it efficient for large-scale problems.

This report presents a partial implementation of the Gu--Eisenstat Divide-and-Conquer SVD algorithm. In section 2, we provide a detailed description of the algorithm, highlighting the solving process of secular equation. Some important implementation details are discussed in section 3, including rational interpolation technique. Finally, section 4 showcases numerical experiments that validate the accuracy and efficiency of our implementation compared to Numpy's built-in SVD function.

\section{Algorithm Description}

Given a matrix \(A \in \C^{m \times n}\), the goal of the Divide-and-Conquer SVD algorithm is to compute its singular value decomposition \(A = U \Sigma V^{\star}\), where \(U \in \C^{m \times m}\) and \(V \in \C^{n \times n}\) are unitary matrices, and \(\Sigma \in \C^{m \times n}\) is a diagonal matrix containing the singular values of \(A\).

\subsection{Bidiagonalization}

The first step is to reduce \(A\) to a bidiagonal form using Householder transformations. This involves applying a series of unitary transformations from the left and right to zero out the appropriate elements, resulting in a matrix \(B\) such that \(A = Q_1 B Q_2^{\star}\), where \(Q_1\) and \(Q_2\) are unitary matrices and \(B\) is lower bidiagonal if \(m \geq n\) or upper bidiagonal if \(m < n\).

Since bidiagonalization is a standard, well-studied process with a complexity of \(O(mn^2)\), it typically dominates the computational cost of the entire SVD. \textbf{To isolate and rigorously evaluate the performance of the Divide-and-Conquer algorithm (which specifically targets the bidiagonal phase), we will bypass this reduction step.} In our implementation and experiments, we will generate random bidiagonal matrices \(B\) directly as inputs.

\subsection{Divide-and-Conquer Strategy}

The core idea of the BDC algorithm is to recursively divide the lower bidiagonal matrix \(B \in \R^{(n + 1) \times n}\) into smaller submatrices, compute their SVDs, and then merge the results to obtain the SVD of the original matrix. Given \(B\), we divide \(B\) into two subproblems as follows:
\[
    B = \begin{bmatrix}
        B_1 & \alpha_k e_k & 0 \\
        0 & \beta_k e_1 & B_2
    \end{bmatrix},
\]
where \(1 < k < n\), \(B_1 \in \R^{k \times (k - 1)}\) and \(B_2 \in \R^{(n - k + 1) \times (n - k)}\) are lower bidiagonal submatrices, and \(e_j\) is the \(j\)-th unit vector of appropriate dimension. The scalars \(\alpha_k\) and \(\beta_k\) are the coupling elements that connect the two submatrices. Usually, we choose \(k = \lfloor n / 2 \rfloor\) to balance the sizes of the subproblems. We then recursively compute the SVDs of \(B_1\) and \(B_2\) and obtain:
\[
    B_i = [Q_i, q_i] \begin{bmatrix}
        D_i \\
        0 \\
    \end{bmatrix} W_i^{\op T}, \quad i = 1, 2.
\]
Notice that
\[
    \begin{aligned}
        \alpha_k e_k &= [Q_1, q_1] \begin{bmatrix}
            Q_1^{\op T} \\
            q_1^{\op T} \\
        \end{bmatrix} \alpha_k e_k = [Q_1, q_1] \begin{bmatrix}
            Q_1^{\op T} e_k \alpha_k \\
            q_1^{\op T} e_k \alpha_k \\
        \end{bmatrix} \\
        \beta_k e_1 &= [Q_2, q_2] \begin{bmatrix}
            Q_2^{\op T} \\
            q_2^{\op T} \\
        \end{bmatrix} \beta_k e_1 = [Q_2, q_2] \begin{bmatrix}
            Q_2^{\op T} e_1 \beta_k \\
            q_2^{\op T} e_1 \beta_k \\
        \end{bmatrix}.
    \end{aligned}
\]
Let \(l_1^{\op{T}}\) and \(\lambda_1\) be the last row and last element of \(Q_1\) and \(q_1\), and similarly define \(f_2^{\op T}\) and \(\varphi_2\) be the first row and first element of \(Q_2\) and \(q_2\). We can rewrite \(B\) as
\[
    B = \begin{bmatrix}
        q_1 & Q_1 \\
        & & Q_2 & q_2 \\
    \end{bmatrix}
    \begin{bmatrix}
        \boxed{\alpha_k \lambda_1} \\
        \alpha_k l_1 & D_1 \\
        \beta_k f_2 &  & D_2 \\
        \boxed{\beta_k \varphi_2}  \\
    \end{bmatrix}
    \begin{bmatrix}
        & W_1 \\
        1 \\
        & & W_2
    \end{bmatrix}^{\op T}
\]
There is only one nonzero element in the first and last columns of the middle matrix. Applying a Givens rotation to zero out \(\beta_k \varphi_2\), we have
\[
    \begin{aligned}
        B &= \begin{bmatrix}
            q_1 & Q_1 \\
            & & Q_2 & q_2 \\
        \end{bmatrix}
        \begin{bmatrix}
            c & & & -s \\
            & I_{k-1} \\
            & & I_{n - k} \\
            s & & & c \\
        \end{bmatrix}
        \begin{bmatrix}
            r_0 \\
            \alpha_k l_1 & D_1 \\
            \beta_k f_2 &  & D_2 \\
            0 \\
        \end{bmatrix}
        \begin{bmatrix}
            & W_1 \\
            1 \\
            & & W_2
        \end{bmatrix}^{\op T} \\
        &= \begin{bmatrix}
            \begin{pmatrix}
                c q_1 & Q_1 \\
                s q_2 & & Q_2 \\
            \end{pmatrix}
            & \begin{pmatrix}
                -s q_1 \\
                c q_2 \\
            \end{pmatrix}
        \end{bmatrix}
        \begin{bmatrix}
            r_0 \\
            \alpha_k l_1 & D_1 \\
            \beta_k f_2 &  & D_2 \\
            0 \\
        \end{bmatrix}
        \begin{bmatrix}
            & W_1 \\
            1 \\
            & & W_2
        \end{bmatrix}^{\op T} \\
        &:= \begin{bmatrix}
            Q & q
        \end{bmatrix}
        \begin{bmatrix}
            M \\
            0 \\
        \end{bmatrix}
        W^{\op T},
    \end{aligned}
\]
where \(c\) and \(s\) are the cosine and sine of the Givens rotation angle, and
\[
    r_0 = \sqrt{(\alpha_k \lambda_1)^2 + (\beta_k \varphi_2)^2}.
\]

Thus, the problem reduces \(\begin{bmatrix}
    M \\ 0 
\end{bmatrix}\) by orthogonal transformations. Note that \(M\) has nonzero elements only on the diagonal and the first column. In next subsection, we will discuss how to compute the SVD of this matrix efficiently and stably. Now, suppose we have computed the SVD of \(M\) as \(M = U \Omega V^{\op T}\), then the SVD of \(B\) can be constructed as
\[
    B = \begin{bmatrix}
        Q & q
    \end{bmatrix} \begin{bmatrix}
        U \Omega V^{\op T} \\
        0 \\
    \end{bmatrix} W^{\op T} := X \begin{bmatrix}
        \Omega \\
        0 \\
    \end{bmatrix} Y^{\op T}.
\]

To compute the SVD of \(B_1\) and \(B_2\), this process can be recursively applied until the submatrices are so small that their SVDs can be computed directly using Golub--Kahan algorithm or other stable methods. There can be at most \(O(\log n)\) levels of recursion.

\subsection{Computing the SVD of \(M\)}

Now we focus on computing the SVD of the matrix
\[
    M = \begin{bmatrix}
        z_1 \\
        z_2 & d_2 \\
        \vdots & & \ddots \\
        z_n & & & d_n \\
    \end{bmatrix}
\]
where \(D = \op{diag}(d_1, \ldots, d_n)\), with \(0 =: d_1 \leq d_2 \leq \cdots \leq d_n\) and \(z = [z_1, \ldots, z_n]^{\op T}\). We further assume that
\[
    d_{j+1} - d_j \geq \tau \|M\|_2, \quad |z_j| \geq \tau \|M\|_2, \quad j = 1, \ldots, n - 1, 
\]
where \(\tau\) is a small positive constant to be specified later. Any matrix of this form can be reduced to one that satisfies these conditions by using the deflation procedure described in the next subsection and a simple permutation.

\begin{lemma} \label{lm:secular}
    Let \(M = U \Omega V^{\op T}\) be the SVD of \(M\) with
    \[
        U = [u_1, \ldots, u_n], \quad V = [v_1, \ldots, v_n], \quad \Omega = \op{diag}(\omega_1, \ldots, \omega_n),
    \]
    Then the singular values \(\omega_i\) satisfy the interlacing property
    \[
        0 < \omega_1 < d_2 < \omega_2 < d_3 < \cdots < d_n < \omega_n < d_n + \|z\|_2,
    \]
    and the secular equation
    \begin{equation} \label{eq:secular}
        f(\omega) := 1 + \sum_{k=1}^{n} \frac{z_k^2}{d_k^2 - \omega^2} = 0.
    \end{equation}
    The singular vectors are given by
    \begin{equation} \label{eq:left_sing}
        u_i = \begin{bmatrix}
            \frac{z_1}{d_1^2 - \omega_i^2} \\
            \frac{z_2}{d_2^2 - \omega_i^2} \\
            \vdots \\
            \frac{z_n}{d_n^2 - \omega_i^2} \\
        \end{bmatrix} \Bigg/ \sqrt{
            \sum_{j=1}^{n} \left( \frac{z_j}{d_j^2 - \omega_i^2} \right)^2
        } \\
    \end{equation}
    and
    \begin{equation} \label{eq:right_sing}
        v_i = \begin{bmatrix}
            -1 \\
            \frac{d_2 z_2}{d_2^2 - \omega_i^2} \\
            \vdots \\
            \frac{d_n z_n}{d_n^2 - \omega_i^2} \\
        \end{bmatrix} \Bigg/ \sqrt{
            1 + \sum_{j=2}^{n} \left( \frac{d_j z_j}{d_j^2 - \omega_i^2} \right)^2
        }
    \end{equation}
\end{lemma}

Given \(D\) and all singular values, we can reconstruct \(M\) using the lemma below.

\begin{lemma} \label{lm:reconstruct}
    Given a diagonal matrix \(D = \op{diag}(d_1, \ldots, d_n)\) and a set of numbers \(\{\hat{\omega_i}\}_{i=1}^{n}\) satisfying the interlacing property
    \[
        0 = d_1 < \hat{\omega}_1 < d_2 < \hat{\omega}_2 < \cdots < d_n < \hat{\omega}_n < d_n + \|z\|_2,
    \]
    there exists a matrix
    \[
        \hat{M} = \begin{bmatrix}
            \hat{z}_1 \\
            \hat{z}_2 & d_2 \\
            \vdots & & \ddots \\
            \hat{z}_n & & & d_n \\
        \end{bmatrix}
    \]
    whose singular values are exactly \(\{\hat{\omega}_i\}_{i=1}^{n}\). The elements \(\hat{z}_i\) can be computed by
    \begin{equation} \label{eq:rank1}
        |\hat{z}_i| = \sqrt{
            (\hat{\omega}_n^2 - d_i^2) \prod_{k=1}^{i-1} \frac{(\hat{\omega}_k^2 - d_i^2)}{(d_k^2 - d_i^2)} \prod_{k=i}^{n} \frac{(\hat{\omega}_k^2 - d_i^2)}{(d_{k+1}^2 - d_i^2)}
        }
    \end{equation}
\end{lemma}

You can refer to Gu and Eisenstat (1995) for the proofs of Lemma \ref{lm:secular} and \ref{lm:reconstruct}.

\subsubsection{Computing the Singular Vectors}

In practice, we can compute the approximate singular values \(\{\hat{\omega}\}_{i=1}^{n}\) by solving the secular equation \eqref{eq:secular}. Once we have the approximate singular values, we can compute the singular vectors using equations \eqref{eq:left_sing} and \eqref{eq:right_sing}. However, directly using these formulas can lead to numerical instability, especially when the singular values are closely spaced --- small perturbations can cause great loss of orthogonality among the computed singular vectors. To address this issue, Gu and Eisenstat (1995) proposed a more stable method for computing the singular vectors.

Lemma \ref{lm:reconstruct} allows us to reconstruct a matrix \(\hat{M}\) whose singular values are exactly the computed approximate singular values \(\{\hat{\omega}_i\}_{i=1}^{n}\). We can then compute the singular vectors of \(\hat{M}\) using equations \eqref{eq:left_sing} and \eqref{eq:right_sing}, which are more stable since the singular values are now exact. This approach significantly improves the numerical stability of the singular vector computation. Thus, the orthogonality of the computed singular vectors can be guaranteed to high precision. We can use \(\hat{M}\)'s SVD as an approximation to \(M\)'s SVD.

\subsubsection{Computing the Singular Values}

Now we discuss how to compute the singular values by solving the secular equation \eqref{eq:secular}. However, if we directly compute \(d_k^2 - \omega^2\), it may lead to severe cancellation errors when \(\omega\) is close to \(d_k\). To avoid this, we can compute the offsets. Specifically, for any \(i \leq n - 1\), we first check whether \(f\left(\frac{d_{i} + d_{i+1}}{2}\right)\) is positive to determine whether the root \(\omega_i\) lies in the left or right half of the interval \((d_i, d_{i+1})\).

First assume that \(\omega_i \in \left(d_i, \frac{d_i + d_{i+1}}{2}\right)\). Now assume the offset \(\mu = \omega_i - d_i\) and the length of the interval \(\delta_j = d_j - d_i\). Then the denominator can be computed as
\[
    d_j^2 - \omega_i^2 = (d_j - d_i - \mu)(d_j + d_i + \mu) = (\delta_j - \mu)(d_i + d_j + \mu).
\]
Note that this method is numerically stable since \(\delta_j - \mu \in (\delta_j/2, \delta_j)\). We will then seek the root \(\mu_i = \omega_i - d_i \in (0, \delta_{i+1}/2)\) in the reformulated secular equation
\[
    f(\mu_i) := 1 + \sum_{k=1}^{n} \frac{z_k^2}{( \delta_k - \mu_i)(d_k + d_i + \mu_i)} = 0.
\]
For the convienience of subsequent discussion, we denote
\[
    \begin{aligned}
        \varphi_i(\mu) &= \sum_{k=1}^{i} \frac{z_k^2}{(\delta_k - \mu)(d_k + d_i + \mu)}, \\
        \psi_i(\mu) &= \sum_{k=i+1}^{n} \frac{z_k^2}{(\delta_k - \mu)(d_k + d_i + \mu)}.
    \end{aligned}
\]

Now assume that \(\omega_i \in \left(\frac{d_i + d_{i+1}}{2}, d_{i+1}\right)\). We set the offset \(\mu_i = d_{i+1} - \omega_i\) and the length of the interval \(\delta_j = d_j - d_{i+1}\). We can similarly give the reformulated secular equation
\[
    f(\mu_i) := 1 + \sum_{k=1}^{n} \frac{z_k^2}{( \delta_k + \mu_i)(d_k + d_{i+1} - \mu_i)} = 0.
\]
We will seek the root \(\mu_i = d_{i+1} - \omega_i \in (0, \delta_{i}/2)\). Similarly, we denote
\[
    \begin{aligned}
        \varphi_i(\mu) &= \sum_{k=1}^{i} \frac{z_k^2}{(\delta_k + \mu)(d_k + d_{i+1} - \mu)}, \\
        \psi_i(\mu) &= \sum_{k=i+1}^{n} \frac{z_k^2}{(\delta_k + \mu)(d_k + d_{i+1} - \mu)}.
    \end{aligned}
\]
Finally we consider the last singular value \(\omega_n > d_n\). Let \(\mu_n = \omega_n - d_n\) and \(\delta_j = d_j - d_n\), and we have the secular equation
\[
    f(\mu_n) := 1 + \sum_{k=1}^{n} \frac{z_k^2}{(\delta_k - \mu_n)(d_k + d_n + \mu_n)} = 0.
\]
And we will seek the root \(\mu_n \in (0, \|z\|_2)\).

We can use bisection or the rational interpolation method to solve these secular equations efficiently. The stopping criterion is set to be
\[
    |f(\mu)| \leq n \ep (1 + |\varphi_i(\mu)| + |\psi_i(\mu)|),
\]

\subsection{Deflation}

\subsubsection{Local Deflation}

Let \(\tau = 2 n^2 \ep\). Now we will show that we can reduce \(M\) to a same form that satisfies
\[
    |d_{i} - d_{j}| \geq \tau \|M\|_2, \quad \forall i \neq j, \quad |z_i| \geq \tau \|M\|_2.
\]

First, we will discuss how to handle the case when some \(z_i\) is very small. There are two subcases:

\begin{enumerate}
    \item If \(|z_1| < \tau \|M\|_2\), then we will replace \(M\) by
    \[
        M = \begin{bmatrix}
            z_1 \\
            z_2 & d_2 \\
            \vdots & & \ddots \\
            z_n & & & d_n \\
        \end{bmatrix} = \begin{bmatrix}
            \tau \|M\|_2 \\
            z_2 & d_2 \\
            \vdots & & \ddots \\
            z_n & & & d_n \\
        \end{bmatrix} + O(\tau \|M\|_2).
    \]
    The perturbed matrix has the same structure as \(M\) and satisfies the condition \(|z_1| \geq \tau \|M\|_2\).

    \item If \(|z_i| < \tau \|M\|_2\), for some \(i \geq 2\), then we will replace \(M\) by
    \[
        M = \begin{bmatrix}
            z_1 \\
            z_2 & d_2 \\
            \vdots & & \ddots \\
            z_n & & & d_n \\
        \end{bmatrix} = \begin{bmatrix}
            z_1 \\
            \vdots & \ddots \\
            0 & & d_i \\
            \vdots & & & \ddots \\
            z_n & & & & d_n \\
        \end{bmatrix} + O(\tau \|M\|_2).
    \]
    In this case, the \(i\)-th singular value of \(M\) is exactly \(d_i\) by Gershgorin circle theorem, and the corresponding left and right singular vectors are \(e_i\). Thus, we can deflate this singular value and reduce the problem size by 1. The principal submatrix maintains the same structure as \(M\).
\end{enumerate}

Next, we will discuss how to handle the case when some \(d_i\) and \(d_j\) are very close. There are two subcases:

\begin{enumerate}
    \item If \(|d_i - d_1| = |d_i| < \tau \|M\|_2\) for \(i \geq 2\), we can set \(d_i\) to zero and zero out the corresponding \(z_i\). Specifically, we can apply a Givens rotation on the rows \(1\) and \(i\) to zero out \(z_i\):
    \[
        GM = \begin{bmatrix}
            c & & s \\
            & I & \\
            -s & & c \\
            & & & & I \\
        \end{bmatrix}
        \begin{bmatrix}
            z_1 \\
            z_2 & d_2 \\
            \vdots & & \ddots \\
            z_n & & & d_n \\
        \end{bmatrix} = \begin{bmatrix}
            r \\
            z_2 & d_2 \\
            \vdots & & \ddots \\
            0 & & & 0 \\
            \vdots & & & & \ddots \\
            z_n & & & & & d_n \\
        \end{bmatrix} + O(\tau \|M\|_2).
    \]
    Thus, the \(i\)-th singular value of \(M\) is exactly zero, and the corresponding left singular vector is \(G^{\op T} e_i\) and right singular vector is \(e_i\). We can deflate this singular value and reduce the problem size by 1. The principal submatrix maintains the same structure as \(M\).
    \item If \(|d_j - d_i| < \tau \|M\|_2\), similarly, we can apply contract Givens rotations on rows \(i\) and \(j\) to zero out \(z_j\):
    \[
        \begin{aligned}
            G M G^{\op T} &= \begin{bmatrix}
            I & & & & \\
            & c & & s & \\
            & & I & & \\
            & -s & & c & \\
            & & & & I \\
        \end{bmatrix}
        \begin{bmatrix}
            z_1 \\
            z_2 & d_2 \\
            \vdots & & \ddots \\
            z_n & & & d_n \\
        \end{bmatrix}
        \begin{bmatrix}
            I & & & & \\
            & c & & -s & \\
            & & I & & \\
            & s & & c & \\
            & & & & I \\
        \end{bmatrix}\\
        &= \begin{bmatrix}
            z_1 \\
            \vdots & \ddots \\
            r & & d_j \\
            \vdots & & & \ddots \\
            0 & & & & d_j \\
            \vdots & & & & & \ddots \\
            z_n & & & & & & d_n \\
        \end{bmatrix} + O(\tau \|M\|_2).
        \end{aligned}
    \]
    In the perturbed matrix, the \(j\)-th singular value is exactly \(d_j\) by Gershgorin circle theorem, and the leading principal submatrix maintains the same structure as \(M\).
\end{enumerate}

This process is called deflation of \(M\) by Gu and Eisenstat, but I think it is more appropriate to call it local deflation since it only considers the elements within \(M\).

\subsubsection{Interblock Deflation}

Note: The following two sections describe the standard strategy proposed by Gu and Eisenstat. In our current simplified implementation, we focus primarily on Local Deflation.

In the dividing strategy, we write
\[
    B = \begin{bmatrix}
        Q & q \\
    \end{bmatrix}
    \begin{bmatrix}
        M \\
        0 \\
    \end{bmatrix}
    W^{\op T},
\]
where
\[
    Q = \begin{bmatrix}
        c_0 q_1 & Q_1 \\
        s_0 q_2 & & Q_2 \\
    \end{bmatrix}, \quad M = \begin{bmatrix}
        r_0 \\
        \alpha_k l_1 & D_1 \\
        \beta_k f_2 &  & D_2 \\
    \end{bmatrix}, \quad W = \begin{bmatrix}
        & W_1 \\
        1 \\
        & & W_2
    \end{bmatrix}.
\]
Note that both \(Q\) and \(W\) are sparse. Thus, if \(M = U \Sigma V^*\), then the main cost is to compute the matrix multiplication \(QU\) and \(WV\). Gu proposed a deflation strategy to reduce the cost of these multiplications. 

If \(r_0\) is small or \((\alpha_k l_1^{\op T}, \beta_k f_2^{\op T})\) has some small elements, we can apply the deflation procedure described in the previous subsection without changing the block structure of \(Q\) and \(W\). If \(D_1\) has a small element or several close elements, we can also apply the deflation procedure to \(D_1\), and similarly for \(D_2\). Again, these cases won't change the block structure of \(Q\) and \(W\).

But if there are some close elements between \(D_1\) and \(D_2\), and we apply the deflation procedure above, then the block structure of \(Q\) and \(W\) will be destroyed. In this case, we can apply a permutation to \(M\). Gu proposed a "polarized permutation" strategy. We will move one of the close elements to the end of \(D_2\) and the other to the start of \(D_1\). This process is just like polarization. Like previous cases, we can apply Givens rotation to zero out the last element of \(\beta_k f_2\):
\[
    G M G^{\op T} = \begin{bmatrix}
        r_0 \\
        r & d_n \\
        \alpha_k l_1' & & D_1' \\
        \beta_k f_2' & &  & D_2' \\
        0 & & & & d_n \\
    \end{bmatrix} + O(\tau \|M\|_2) = \begin{bmatrix}
        M' \\
        & d_n \\
    \end{bmatrix} + O(\tau \|M\|_2).
\]
As a cost, this deflation will still change the block structure of \(Q\) and \(W\). However, since we have placed the close elements at the boundary of \(D_1\) and \(D_2\), we can still maintain most of the sparsity of \(Q\) and \(W\). Specifically, we have
\[
    \begin{aligned}
        B &= \begin{bmatrix}
        Q G^{\op T} & q \\
    \end{bmatrix}
    \begin{bmatrix}
        G M G^{\op T} \\
        0 \\
    \end{bmatrix}
    (W G^{\op T})^{\op T} \\
    &= \begin{bmatrix}
        X_1 & x & q
    \end{bmatrix}
    \begin{bmatrix}
        M' \\
        & d_n \\
        0 & 0 \\
    \end{bmatrix}
    \begin{bmatrix}
        Y_1 & y \\
    \end{bmatrix}^{\op T} + O(\tau \|M\|_2),
    \end{aligned}
\]
where
\[
    X_1 = \begin{bmatrix}
        c_0 q_1 & c q_1' & Q_1' & 0 \\
        s_0 q_2 & s q_2' & 0 & Q_2' \\
    \end{bmatrix}, \quad
    x = \begin{bmatrix}
        -s q_1' \\
        c q_2' \\
    \end{bmatrix}
\]
and
\[
    Y_1 = \begin{bmatrix}
        0 & c w_1' & W_1' & 0 \\
        1 & 0 & 0 & 0 \\
        0 & s w_2' & 0 & W_2' \\
    \end{bmatrix}, \quad y = \begin{bmatrix}
        -s w_1' \\
        0 \\
        c w_2' \\
    \end{bmatrix}.
\]

\(d_n\) is an approximate singular value that can be deflated. The corresponding left singular vector is \(x\) and right singular vector is \(y\). The matrix \(M_1\) has the same structure as \(M\) and can be deflated in a similar fashion until no diagonal element of \(D_1\) is close to a diagonal element of \(D_2\).

\subsubsection{Global Deflation}

We look back to the whole BDC algorithm. To illustrate global deflation, we look at two levels of the dividing strategy. 
\[
    B = \begin{bmatrix}
        B_1 & \alpha_{i+j} e_{i+j} & 0 \\
        0 & \beta_{i+j} e_1 & B_2 \\
    \end{bmatrix} = \begin{bmatrix}
        B_{1,1} & \alpha_i e_i & 0 & 0 & 0 \\
        0 & \beta_i e_1 & B_{1,2} & \alpha_{i+j} e_j & 0 \\
        0 & 0 & 0 & \beta_{i+j} e_1 & B_2 \\
    \end{bmatrix}
\]
Let \(X_{1,2} \begin{bmatrix}
    D_{1,2} \\
    0 \\
\end{bmatrix} Y_{1,2}^{\op T}\) be the SVD of \(B_{1,2}\), and let \((f_{1,2}^{\op T}, \varphi_{1,2})\) and \((l_{1,2}^{\op T}, \lambda_{1,2})\) be the first and last rows of \(X_{1,2}\), respectively. Then
\[
    B = \tilde{X} \begin{bmatrix}
        B_{1,1} & \alpha_i e_i & 0 & 0 & 0 \\
        0 & \beta_i f_{1,2} & D_{1,2} & \alpha_{i+j} l_{1,2} & 0 \\
        0 & \beta_i \varphi_{1,2} & 0 & \alpha_{i+j} \lambda_{1,2} & 0 \\
        0 & 0 & 0 & \beta_{i+j} e_1 & B_2 \\
    \end{bmatrix} \tilde{Y}^{\op T},
\]
Let $\bar{d}_s$ be the $s$th diagonal element of $D_{1,2}$, and let $\bar{f}_s$ and $\bar{l}_s$ be the $s$th components of $f_{1,2}$ and $l_{1,2}$, respectively. Then ignoring all zero components, the $(i+s)$th column and row of the middle matrix are $(\bar{d}_s)$ and $(\beta_i\bar{f}_s, \bar{d}_s, \alpha_{i+j}\bar{l}_s)$, respectively. Thus if both $|\beta_i\bar{f}_s|$ and $|\alpha_{i+j}\bar{l}_s|$ are small, then we can perturb them to zero. $\bar{d}_s$ is a singular value of the perturbed matrix and the $(i+s)$th columns of $\tilde{X}$ and $\tilde{Y}$ are the corresponding left and right singular vectors, respectively. This singular value and its singular vectors can be deflated from all subsequent subproblems. We call this global deflation.

\section{Implementation Details}

My code structure is as follows:
\begin{lstlisting}[caption={Core Project Structure Snippet}]
src/
|-- __init__.py
|-- dnc_svd.py
|-- secular.py
|-- utils.py
tests/
|-- test_secular.py
report/
|-- report.tex
|-- report.pdf
\end{lstlisting}

In \texttt{dnc\_svd.py}, I implemented the BDC algorithm. In \texttt{secular.py}, I implemented the secular equation solver with local deflation. There is no need to talk about \texttt{dnc\_svd.py}, since the main content has been discussed in previous sections. What I want to highlight is the implementation of \texttt{secular.py}. In this part, we use rational interpolation to solve the secular equation. More precisely, we use fractional forms to approximate the secular function \(f(\omega)\):
\[
    f(\omega) \approx \frac{C_1}{\omega - d} + C_2.
\]
Here, \(d\) is the nearest pole to the current approximation of the root. \(C_1\) and \(C_2\) can be computed by evaluating \(f(\omega)\) and its derivative at the current approximation. Then we can compute the next approximation by solving the equation
\[
    \frac{C_1}{\omega - d} + C_2 = 0.
\]
This method converges very fast when the initial approximation is good. But sometimes it can lead to points outside the interval. To avoid this, we combine \textbf{rational interpolation} and \textbf{bisection} together. In most cases, we use rational interpolation. If the next approximation is outside the interval, we use bisection to get the next approximation.

\section{Numerical Experiments}

Sorry that my implementation of the BDC algorithm is full of bugs and I haven't fixed them all yet. Thus, I cannot provide numerical experiments for BDC now. But, I have tested the secular equation solver with local deflation. The results are shown below.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./figures/secular_function.pdf}
\end{figure}

The above figure shows the secular function \(f(\omega)\) for an arrowhead matrix. The green dots are the computed singular values. As we can see, the secular function is very close to zero at these points, which indicates that our secular equation solver works well. This experiment can be seen in \texttt{exp\_secular.py} in experiments folder.

Another example of the secular equation solver is shown below:
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./figures/test_secular.png}
    \caption{Numerical experiment for secular equation solver. The test has five different phases. The first one is standard case. The second one has two small \(z\) elements. The third one has two close \(d\) elements. The fourth one intentionally creates unsorted \(d\) elements to test the permutation. The last one tests the general numerical accuracy by comparing \(f(\omega)\) with zero at computed singular values.}
\end{figure}

You can see this experiment in \texttt{test\_secular.py} in test folder.

In fact, I have tested the main solver \texttt{DCSVDSolver} in \texttt{dnc\_svd.py} as well, but there exist huge errors in the computed singular values. I don't know why. I have turned to AI-s, but none of them can help me fix the bugs. The results are shown below:
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./figures/svd_accuracy.pdf}
    \caption{Numerical experiment for BDC algorithm.}
\end{figure}
We can observe that the main error occurs in the largest singular values. The problem can't be in the secular equation solver, since the same size of problem can be solved accurately there. I suspect that the problem lies in the dividing strategy or the merge strategy. But I have no idea how to fix it now. I will keep working on it in the future.

Although the BDC algorithm is not working now, the main framework is ready. So we can test the runtime of the algorithm. You can see the results below:
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./figures/svd_runtime_comparison.pdf}
    \caption{Runtime experiment for BDC algorithm.}
\end{figure}

Although the theoretical worst-case complexity for computing singular vectors is $O(n^3)$, our experiments exhibit a runtime complexity closer to $O(n^2)$. This empirically demonstrates the effectiveness of the deflation strategy implemented in our solver, which significantly reduces the dimensionality of the secular equations for the test matrices. But you can still see that there exist an increase of slope when the matrix size goes to 1000. This indicates that the algorithm is actually \(O(n^3)\).

\section{Conclusion}

In this report, we have discussed the divide-and-conquer algorithm for computing the singular value decomposition of a matrix. We have explained the main ideas of the algorithm, including the dividing strategy, the merge strategy, and the secular equation solver with local deflation. We have also provided some implementation details and numerical experiments.

\section{Future Work}

There are several directions for future work:
\begin{enumerate}
    \item Fix the bugs in the BDC algorithm implementation and ensure its correctness.
    \item Implement interblock deflation and global deflation to improve the efficiency of the algorithm.
    \item Conduct more extensive numerical experiments to evaluate the performance of the BDC algorithm compared to other SVD algorithms.
\end{enumerate}

\end{document}